...
void showAdditionalImageDialog(BuildContext context, String url) {
  showDialog(
    context: context,
    builder: (_) => Dialog(
      child: Column(
...
Widget buildAdditionalImageTile(
  String url, {
...
/// Renders a horizontal row of additional image tiles.
///
/// Pass [onRemove] to show a remove cross on each tile.
/// Pass [onAdd] to show an "Add Image" button after the tiles.
/// Returns [SizedBox.shrink] if [urls] is empty and [onAdd] is null.
Widget buildAdditionalImagesRow(
  BuildContext context,
  List<String> urls, {
  Set<int> uploadingIndices = const {},
  void Function(int index)? onRemove,
  VoidCallback? onAdd,
}) {
  if (urls.isEmpty && onAdd == null) return const SizedBox.shrink();
  return Column(
        ...
        SingleChildScrollView(
          ...
          child: Row(
            children: urls.asMap().entries.map((entry) {
              ...
              return buildAdditionalImageTile(
                url,
                isUploading: isUploading,
                onTap: () => showAdditionalImageDialog(context, url),
